...
impl<T: CuDataType> Drop for CuVector<T> {
    fn drop(&mut self) {
        cuda_free(self.deref.ptr as *mut c_void)
    }
}

impl<T: CuDataType> Deref for CuVector<T> {
    type Target = CuVectorDeref<T>;
    fn deref(&self) -> &CuVectorDeref<T> {
        &self.deref
    }
}
impl<T: CuDataType> DerefMut for CuVector<T> {
    fn deref_mut(&mut self) -> &mut CuVectorDeref<T> {
        &mut self.deref
    }
}
